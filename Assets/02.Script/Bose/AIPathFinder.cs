...
public class AIPathFinder : MonoBehaviour
{
    ...
    float PathFindingTimer = 2f; // 2초마다 경로 갱신
    ...
    void Update()
    {
        ...
        if (FindPathList.Count == 0)// || PathFindingTimer <= 0f)
        {
            PathFinding();
            if (FindPathList.Count > 0)
            {
                this.GetComponent<BoseCtrl>().MoveNextPos = new Vector3(FindPathList[0].x, FindPathList[0].y, 0f);
            }
            PathFindingTimer = 2f;
        }
    }
